...
## Function to map keyboard keys to drone control and map position
...
## Function to draw points of drone movement on map
def drawPoints(img, points, markPoints):
    for point in points:
        cv2.circle(img, point, 5, (0, 0, 255), cv2.FILLED) ## draw every point in drone's path
    ...
    for mpoint in markPoints:
        cv2.circle(img, mpoint, 8, (255, 0, 0), cv2.FILLED) ## draw marked points
        cv2.putText(img, f'({(mpoint[0] - 500) / 100}, {-(mpoint[1] - 500) / 100})m',
                    (mpoint[0] + 10, mpoint[1] + 30), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 0), 1) ## write coordinates
...
while True:
    ...
    img = cv2.resize(img, (360, 240)) ## resize img to set width and height
    ...
    if (points[-1][0] != vals[4] or points[-1][1] != vals[5]):
        ...
        if kp.getKey('z'):
            markPoints.append((vals[4], vals[5])) ## add marked point when z button is pressed
    drawPoints(map, points, markPoints) ## draw point of drone position on map
    cv2.imshow('Image', img) ## display image
    cv2.imshow('Map', map) ## display map
    cv2.waitKey(1)
